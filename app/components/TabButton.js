        ...
        >
            ...
            >
                <Animated.View style={[
                    ...
                    borderRadius: 100,
                    position: 'absolute',
                    backgroundColor: item.icon === 'alert' ? 'red' : '#0792b3',
                    },
                    scaleStyles
                ]}/>
                    <Material
                        name={item.icon}
                        color={accessibilityState.selected ? '#fff' : item.icon === 'alert' ? 'red' : '#0792b3'}
                        size={22}
                    />            
                </Animated.View>
            <Animated.Text style={[styles.title, {opacity: scale}, {color: item.icon === 'alert' ? 'red' : '#0792b3'}]}>{item.title}</Animated.Text>
        </TouchableOpacity>
    )
}
